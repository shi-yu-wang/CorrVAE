...
ADDITIONAL_EXP = ['custom', "debug", "best_dsprites"]
...
def parse_arguments(args_to_parse):
    ...
    model.add_argument('-num_prop', type=int,
                       default=default_config['num_prop'],
                       help='number of properties to be learnt.')
...
#         # PREPARES DATA
#         train_loader = get_dataloaders(args.dataset,train_sampler,
...
#         model = init_specific_model(args.model_type, args.img_size, args.latent_dim, args.num_prop)
#         logger.info('Num parameters in model: {}'.format(get_n_param(model)))

#         # TRAINS
#         if args.model_type=='CsVAE':
#             optimizer1 = optim.Adam(chain(model.encoder.parameters(),model.decoderX.parameters()), lr=args.lr)
#             optimizer2 = optim.Adam(model.decoderY.parameters(), lr=args.lr)
...
#         print('n_data is:'+str(len(train_loader.dataset)))
...
#         model = load_model(exp_dir, is_gpu=not args.no_cuda, epoch=args.epochs)
#         metadata = load_metadata(exp_dir)
#         # TO-DO: currently uses train datatset
#         test_loader = get_dataloaders(args.dataset,test_sampler,
...
#                             device=device,
#                             **vars(args))
#         evaluator = Evaluator(model, loss_f,args.model_type,
#                               device=device,
#                               logger=logger,
#                               save_dir=exp_dir,
#                               is_progress_bar=not args.no_progress_bar,
...
#         metric=evaluator(test_loader, is_metrics=args.is_metrics, is_losses=not args.no_test)
#         return metric
